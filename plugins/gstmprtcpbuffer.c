		...
		guint32 ext_hsn, guint32 jitter, guint32 LSR, guint32 DLSR)
{
  ...
  gst_rtcp_header_getdown(&riport->header, NULL, NULL, &rc, NULL, &length, NULL);
  ...
  gst_rtcp_header_change(&riport->header, NULL, NULL, &rc, NULL, &length, NULL);
}
		...
		)
{
    ...
    block->cum_packet_lost = (cum_packet_lost & 0x000000FF)<<16 |
    		                 (cum_packet_lost & 0x0000FF00) |
			                 (cum_packet_lost & 0x00FF0000)>>16;
...
void gst_rtcp_xr_rfc7243_setup(
		GstRTCPXR_RFC7243 *riport, guint8 interval_metric,
...
void gst_rtcp_xr_rfc7243_change(
		GstRTCPXR_RFC7243 *riport, guint8 *interval_metric,
		gboolean *early_bit, guint32 *ssrc, guint32 *discarded_bytes)
{
	if(discarded_bytes){
...
void gst_rtcp_xr_rfc7243_getdown(
		GstRTCPXR_RFC7243 *riport, guint8 *interval_metric,
		gboolean *early_bit, guint32 *ssrc, guint32 *discarded_bytes)
{
	if(discarded_bytes){
  ...
  for(; ok; header=(guint8*) header + offset){
	...
	switch(payload_type){
	  ...
	  case GST_RTCP_TYPE_XR:
		gst_print_rtcp_xr((GstRTCPXR_RFC7243*)header);
		break;
	  default:
		  ok = FALSE;
		  break;
...
void gst_print_mprtcp(GstMPRTCPSubflowRiport *riport)
{
  ...
  for(index = 0; index < src; ++index){
	  ...
	  block = ((guint8*) block) + ((block_length + 1)<<2);
  }
}
...
void gst_print_rtcp_xr(GstRTCPXR_RFC7243 *riport)
{
	...
	gst_print_rtcp_header(&riport->header);
    gst_rtcp_xr_rfc7243_getdown(riport, &interval_metric, &early_bit, &ssrc, &discarded_bytes);
	g_print(
		   "+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+\n"
		   "|%15d|%3d|%1d|%9d|%31d|\n"
		   "+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+\n"
