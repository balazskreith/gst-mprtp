    ...
    Subflow *subflow)
{
  ...
  if(--subflow->retention == 0){
    ...
    if(subflow->consecutive_stable < 4){
      ...
      else if(subflow->sending_rate < subflow->fallen_from)
        _setup_monitoring(subflow, 8);
      else
...
_transit_to(
    ...
    State target)
{
  if(target == STATE_MONITORED){
    if(this->monitoring)
      target = STATE_STABLE;
  }else{
    if(subflow->state == STATE_MONITORED)
