...
plot statfile using ($0*0.1):(($10+$11)/125) with point pointtype 7 ps 0.3 lc rgb "blue" title "Flow 2 Subflow 1 (SR + FEC)", \
     statfile using ($0*0.1):9 with lines lc rgb "0xDC143C" lw 2 title "Path 1 Capacity", \

set format x "%3.0f"
set xtics font ", 26"
set xlabel "Time [s]" offset 0,-1.1 font ", 22"

plot statfile using ($0*0.1):(($14+$15)/125) with point pointtype 7 ps 0.3 lc rgb "0x008c48" title "Flow 2 Subflow 2 (SR + FEC)", \
     statfile using ($0*0.1):13 with lines lc rgb "0xf47d23" lw 2 title "Path 2 Capacity"
