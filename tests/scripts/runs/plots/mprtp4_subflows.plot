...
plot statfile using ($0*0.1):(($2+$3)/125) with point pointtype 7 ps 0.3 lc rgb "blue" title "Flow 1 Subflow 1 (SR + FEC)", \
	 statfile using ($0*0.1+20):(($6+$7)/125) with point pointtype 7 ps 0.3 lc rgb "0x008c48" title "Flow 2 Subflow 1 (SR + FEC)", \
     statfile using ($0*0.1):1 with lines lc rgb "0xDC143C" lw 2 title "Path 1 Capacity", \
     
plot statfile using ($0*0.1):(($9+$10)/125) with point pointtype 7 ps 0.3 lc rgb "blue" title "Flow 1 Subflow 2 (SR + FEC)", \
	 statfile using ($0*0.1+20):(($12+$13)/125) with point pointtype 7 ps 0.3 lc rgb "0x008c48" title "Flow 2 Subflow 2 (SR + FEC)", \
