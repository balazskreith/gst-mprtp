...
plot statfile using ($0*0.1):(($2+$3+$8+$9)/125) with point pointtype 7 ps 0.3 lc rgb "0x662c91" title "Total (SR + FEC)", \
	 statfile using ($0*0.1):(($5+$11)/125) with point pointtype 7 ps 0.3 lc rgb "0xFF6347" title "TCP Rate"
...
plot statfile using ($0*0.1):(1) with filledcurves x1 lc rgb "0x008c48" title "Subflow 2", \
     statfile using ($0*0.1):(($2+$3)/($2+$3+$8+$9)) with filledcurves x1 lc rgb "blue" title "Subflow 1"
