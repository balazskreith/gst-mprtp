...
./statmaker $LOGSDIR/tcprate_1.csv tcpstat $LOGSDIR/tcps_1.pcap
tar -cvzf $LOGSDIR/tcps_1.tgz $LOGSDIR/tcps_1.pcap
rm $LOGSDIR/tcps_1.pcap

./statmaker $LOGSDIR/tcprate_2.csv tcpstat $LOGSDIR/tcps_2.pcap
tar -cvzf $LOGSDIR/tcps_2.tgz $LOGSDIR/tcps_2.pcap
rm $LOGSDIR/tcps_2.pcap

paste -d, $LOGSDIR/pathbw_1.csv $LOGSDIR/sr_s1.csv $LOGSDIR/qmd_s1.csv \
  $LOGSDIR/tcprate_1.csv \
  $LOGSDIR/pathbw_2.csv $LOGSDIR/sr_s2.csv $LOGSDIR/qmd_s2.csv \
  $LOGSDIR/tcprate_2.csv > $LOGSDIR/plotstat.csv 

gnuplot -e "statfile='temp/plotstat.csv'" \
        -e "output_file='$PLTOFILE_SUBFLOWS'" \
        -e "path_delay='$PATH_DELAY'" \
        $SCRIPTSDIR/runs/plots/mprtp6_subflows.plot
        
gnuplot -e "statfile='temp/plotstat.csv'" \
        -e "output_file='$PLTOFILE_AGGR'" \
        -e "path_delay='$PATH_DELAY'" \
        $SCRIPTSDIR/runs/plots/mprtp6_aggr.plot
